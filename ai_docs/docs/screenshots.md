...
`grok-bot-predictions.png` was supplied by the product owner on 2026-09-07 as `Screenshot 2026-09-07 at 12.36.54.png`, identified as Gina MCP prediction output in Grok Bot. The original 1484 × 1446 PNG is preserved unchanged (311,707 bytes). It shows Gina Predictoor and a Prediction Market Ops table dated August 30, 2026, including links, sides, odds, liquidity, times, and qualitative assessments. No credentials are visible. The MCP tool call, endpoint, scopes, authentication, and installation route are outside the capture; this is owner-supplied result evidence, not an independently executed connection test or proof of a marketplace listing. Included in `docs/connect/grok-bot.mdx`.

`automations-overview.png` was supplied by the product owner on 2026-09-07 as `Screenshot 2026-09-07 at 12.11.30.png`. The original 2774 × 2594 light-mode PNG is preserved unchanged (919,537 bytes). It shows the Gina web app Automations overview with totals, search, type/status filters, New automation and New bundle controls, selection controls, and automation cards with schedules, run counts, and status. Automation names and activity are visible; their sample/live status was not specified. No credentials or trigger URLs are visible. No automation was created, enabled, disabled, deleted, or run during documentation work. Included in `docs/product-guide/recipes-and-webhooks.mdx`. The Automations list capture is complete; recipe, schedule, webhook, and copy-trading detail captures remain separate pending items.

`credits-purchase.png` was supplied by the product owner on 2026-09-07 as `Screenshot 2026-09-07 at 12.07.57.png`. The original 1174 × 2322 light-mode PNG is preserved unchanged (316,392 bytes). It shows Buy credits with an amount selection, credit balance, usage estimates, USDC payment selection and estimated spend, enabled Auto top-up, Buy button, and historical credit purchases. Account values and purchase history are visible; their sample/live status was not specified. No credentials are visible. No purchase or setting change was performed during documentation work. Included in `docs/product-guide/networks-fees-and-pricing.mdx`. Credits and cryptoasset purchase-review capture is complete. Displayed estimates are not independently verified pricing or usage guarantees.

`agent-setup-full-access.png` was supplied by the product owner on 2026-09-07 as `Screenshot 2026-09-07 at 12.07.05.png`. The original 2272 × 2608 light-mode PNG is preserved unchanged (580,402 bytes). It shows the wallet-access notice, Full access — view and execute selection, empty Active Tokens section, and the visible portion of the venue-specific OpenClaw Setup template. The authorization value is a placeholder; no token secret is visible. No token was generated during documentation work. Included in `docs/agents/authentication.mdx`. MCP capture #8 (full-access Agent Setup) is complete. The product owner confirmed on 2026-09-07 that general account-overview and spot widgets do not exist; neither is a pending capture.

`chatgpt-perps-positions.png` was supplied by the product owner on 2026-09-07 as `Screenshot 2026-09-07 at 12.05.41.png`, identified as the ChatGPT perps positions widget. The original 1814 × 1556 PNG is preserved unchanged (356,730 bytes). It shows the prompt asking for perp positions, one BTC long position at 3x, aggregate P&L, exposure, margin, holding size, entry price, funding, and unavailable liquidation information. Account values are visible; their sample/live status was not specified. No credentials are visible. The CSP off badge is preserved, and the assistant summary is recorded without independently verifying its venue count. No financial action was performed during documentation work. Included in `docs/read-only/perps.mdx`. This adds position coverage to the existing perps market-chart capture; the general account-overview widget is unavailable, as confirmed by the product owner.

`claude-connector.png` was supplied by the product owner on 2026-09-07 as `Screenshot 2026-09-07 at 11.58.08.png`. The original 1740 × 948 PNG is preserved unchanged (163,641 bytes). It shows Claude Customize → Connectors, with Gina under Your custom connectors, the askgina.ai domain, a Custom label, and a green checkmark. Host branding is visible and the owner identifies Gina as added. No credentials are visible. This records the custom connector listing, not its exact MCP endpoint, granted scopes, or the authentication flow. Included in `docs/connect/claude.mdx`. MCP capture #2 (host connection) is complete.

`chatgpt-prediction-game-detail.png` was supplied by the product owner on 2026-09-07 as `Screenshot 2026-09-07 at 11.57.14.png`. The original 2216 × 2502 PNG is preserved unchanged (915,549 bytes). It shows ChatGPT's dark-mode expanded Ask-Gina view for Manchester United versus Manchester City, with outcome prices, chart, grouped game markets, related fixtures, and Ask about this match / Open in Ask Gina controls. ChatGPT host chrome is visible. No credentials are visible. Prices, market groupings, and times are recorded as displayed; the displayed match time differs from the earlier game-card capture and was not independently verified. No widget controls were exercised during documentation work. Included in `docs/read-only/predictions.mdx` alongside the sports game-card example.

`chatgpt-prediction-sports.png` was supplied by the product owner on 2026-09-07 as `Screenshot 2026-09-07 at 11.53.02.png`, identified by the owner as ChatGPT. The 1702 × 1888 dark-mode capture shows a Premier League game-market request and cards grouped by Saturday and Sunday, with kickoff times, Etc/UTC timezone, team/draw outcome prices, and market dates. The original exceeds the 1 MB docs limit, so the PNG is compressed losslessly; decoded RGBA pixels are verified identical. No credentials are visible. The CSP off badge is preserved; this does not establish production CSP configuration or connection scopes. Fixtures and market data are recorded as shown, without independent schedule verification. Included in `docs/read-only/predictions.mdx` as an additional sports prediction-widget example.
...
| Prediction and perps MCP widgets (captured)       | Relevant `read-only/` guide and supported host guide       |
