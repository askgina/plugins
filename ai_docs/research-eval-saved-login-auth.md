...
| --- | --- | --- | --- |
| Native Codex CLI 0.153.4 | Codex ChatGPT OAuth | Local `codex exec` reuses the same home's saved login. A separate `CODEX_HOME` needs a file-backed `auth.json` copy and serialized refresh write-back, and OpenAI forbids that CI copy on public or open-source repositories. | **SUPPORTED, with mutable-store custody** |
...
| AI SDK Harness Codex 1.0.104 | None in adapter contract | API key or AI Gateway variables only. Brokered sandboxes get a placeholder; unbrokered sandboxes receive the real key. | **UNSUPPORTED for saved login** |
| AI SDK Harness Claude Code 1.0.106 | None first-class | API key, bearer, Gateway, or host `apiKeyHelper`. Extra `env` can reach the bridge unbrokered. | **No first-class saved-login path; unbrokered token exposure UNKNOWN** |
...
Native local reuse does not copy `auth.json`. On a machine that already ran `codex login`, `codex exec` uses the saved ChatGPT session in that process's `CODEX_HOME`. No file export is required, and refresh writes back into that same store.

OpenAI documents a separate advanced CI handoff only for trusted private automation: create file-backed `auth.json` with Codex, restore it to the runner, run Codex normally, then persist the refreshed file. Use one `auth.json` per machine or serialized job stream. Do not share the same copy across concurrent jobs. The guide states: "Do not use this workflow for public or open-source repositories." Publishing this research does not authorize applying that CI saved-auth workflow to a public repo, including this one. The handoff is a credential copy with mutable write-back, not a read-only mount. [[CI/CD auth](https://learn.chatgpt.com/docs/auth/ci-cd-auth)]
...
These experimental adapters are pinned to Vercel AI commit `c3c189c`. The Codex adapter resolves only API key/Gateway inputs. It has no `CODEX_HOME`, saved-store, or `CODEX_ACCESS_TOKEN` setting. When the sandbox supports `addRequestTransformations`, the bridge receives a placeholder `CODEX_API_KEY` and the host rewrites matching outbound `Authorization` headers. Sandboxes without that hook get direct credential forwarding into the bridge process. [[Codex auth source](https://github.com/vercel/ai/blob/c3c189c068baefe62808b4eca5432fe092dc37cb/packages/harness-codex/src/codex-auth.ts)] [[Codex harness start](https://github.com/vercel/ai/blob/c3c189c068baefe62808b4eca5432fe092dc37cb/packages/harness-codex/src/codex-harness.ts)]

The Claude adapter has no first-class or brokered saved-login path. It resolves API key, bearer, Gateway, and base URL values. In default auto mode it may read `~/.claude/settings.json` and execute `apiKeyHelper` in the host process. An explicit authentication record stops ambient discovery. The broker allowlist is `AI_GATEWAY_API_KEY`, `ANTHROPIC_API_KEY`, and `ANTHROPIC_AUTH_TOKEN`. Adapter `env` is merged onto the bridge process after resolved auth, so a non-allowlisted value such as `CLAUDE_CODE_OAUTH_TOKEN` can reach the bridge unbrokered if the caller puts it there. That is not a supported saved-login contract. Whether it leaks into model-callable tools remains **UNKNOWN**. The credential-forwarding callback controls sandbox values, not host discovery. [[Claude auth source](https://github.com/vercel/ai/blob/c3c189c068baefe62808b4eca5432fe092dc37cb/packages/harness-claude-code/src/claude-code-auth.ts)] [[adapter docs](https://ai-sdk.dev/providers/ai-sdk-harnesses/claude-code#adapter-settings)]
