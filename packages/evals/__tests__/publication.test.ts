...
const canonicalAttemptId = (caseId: string, repetition: number): string =>
  `attempt-${sha256Hex(JSON.stringify([RUN_ID, caseId, repetition]))}`;
const FABRICATED_ATTEMPT_ID = `attempt-${sha256Hex("fabricated-attempt-identity")}`;
...
const makeResult = (
  ...
  ranking: {
    status: "unranked",
    reasons: dataOrigin === "synthetic" ? ["pilot", "synthetic"] : ["pilot"],
  },
  attempts: null,
  ...overrides,
});
...
const makeAttempt = (
  ...
  id: canonicalAttemptId(caseId, repetition),
  runId: RUN_ID,
  caseId,
  repetition,
  verdict,
  ...
  checks: verdict === "fail" ? { ...passChecks, routing: "fail" } : passChecks,
  failureCategories: verdict === "fail" ? ["routing_mismatch"] : [],
  durationMs,
  tokenUsage: null,
  replacementOf: null,
});

const detailedAttempts: PublicEvalAttemptSummary[] = [
  makeAttempt("case-a", 1, "pass", 5),
  makeAttempt("case-a", 2, "pass", 5),
  makeAttempt("case-b", 1, "pass", 7),
  makeAttempt("case-b", 2, "fail", 7),
];

const detailedResult = makeResult("synthetic", {
  source: {
    kind: "sanitized_aggregate_with_attempts",
    reportSchemaVersion: "v1",
    reportSha256: sha256Hex("synthetic-report"),
    attemptCaptureSha256: sha256Hex("synthetic-attempts"),
  },
  counts: {
    attempts: { total: 4, passed: 3, failed: 1 },
...
const correction = makePublication({
  ...
  supersedes: {
    ...
    revision: 1,
    reason: "correction",
    summary: "Corrected the latency percentiles.",
  },
...
const withdrawal = makePublication({
  ...
  supersedes: {
    ...
    revision: 2,
    reason: "withdrawal",
    summary: "Withdrawn after a privacy review.",
  },
  content: {
...
const errorText = (error: PublicEvalPublicationError): string =>
  `${String(error)} ${JSON.stringify(error)}`;

const PrettyUnknownJson = Schema.fromJsonString(Schema.Unknown, { space: 2 });
const decodePrettyUnknownJson = Schema.decodeEffect(PrettyUnknownJson);
const prettyJsonFile = (value: unknown) =>
  Schema.encodeUnknownEffect(PrettyUnknownJson)(value).pipe(
    Effect.map((encoded) => `${encoded}\n`),
  );

class PublicationTamperError extends Data.TaggedError("PublicationTamperError")<{}> {}

const readJson = (fs: FileSystem.FileSystem, target: string) =>
  fs.readFileString(target).pipe(Effect.flatMap(decodePrettyUnknownJson));
...
const MutableIndexJson = Schema.fromJsonString(
  Schema.Struct({
    schemaVersion: Schema.Literal("eval-index.v1"),
    dataOrigin: Schema.Literals(["measured", "synthetic"]),
...
type MutableIndex = {
  schemaVersion: (typeof MutableIndexJson.Type)["schemaVersion"];
  dataOrigin: (typeof MutableIndexJson.Type)["dataOrigin"];
  generatedAt: string;
  publications: Array<{
    publicationId: string;
    runId: string;
    review: (typeof MutableIndexJson.Type)["publications"][number]["review"];
    status: (typeof MutableIndexJson.Type)["publications"][number]["status"];
    ...
    revisions: Array<{
      revisionId: string;
      revision: number;
      kind: (typeof MutableIndexJson.Type)["publications"][number]["revisions"][number]["kind"];
      state: (typeof MutableIndexJson.Type)["publications"][number]["revisions"][number]["state"];
      publishedAt: string;
      path: string | null;
      sha256: string | null;
    }>;
...
const readMutableIndex = (fs: FileSystem.FileSystem, target: string) =>
  fs.readFileString(target).pipe(
    Effect.flatMap(Schema.decodeEffect(MutableIndexJson)),
    Effect.map((index): MutableIndex => ({
      schemaVersion: index.schemaVersion,
      dataOrigin: index.dataOrigin,
      generatedAt: index.generatedAt,
      publications: index.publications.map((entry) => ({
        ...
        revisions: entry.revisions.map((revision) => ({ ...revision })),
      })),
    })),
  );
...
describe("public eval publication exporter", () => {
  it.layer(TestPlatformLayer)((it) => {
    it.effect(
      "publishes a synthetic preview and appends a correction without touching the first snapshot",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
          ...
          assert.deepStrictEqual(yield* decodePrettyUnknownJson(firstBytes), initial);
          ...
          assert.deepStrictEqual(yield* decodePrettyUnknownJson(secondBytes), correction);

          const corrected = yield* readPublicEvalIndex({
            outputDirectory,
    ...
    it.effect(
      "rejects a well-formed noncanonical attempt id and publishes the canonical detailed result",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
          const { outputDirectory } = yield* canonicalOutputDirectory("publication-attempt-id-");
          const origin = originPaths(path, outputDirectory);
          const fabricatedAttempts = detailedAttempts.map((attempt, index) =>
            index === 0 ? { ...attempt, id: FABRICATED_ATTEMPT_ID } : attempt,
          );
          const fabricated = makePublication({
            revisionId: "rev-1",
            revision: 1,
            publishedAt: PUBLISHED_1,
            content: {
              kind: "result",
              result: { ...detailedResult, attempts: fabricatedAttempts },
            },
          });
          yield* rejection(
            exportTo(outputDirectory, fabricated),
            "invalid_publication",
            "fabricated attempt id",
          );
          ...
          const detailed = makePublication({
            revisionId: "rev-1",
            revision: 1,
            publishedAt: PUBLISHED_1,
            content: { kind: "result", result: detailedResult },
          });
          const published = yield* exportTo(outputDirectory, detailed);
          const bytes = yield* fs.readFileString(published.publicationPath);
          assert.deepStrictEqual(yield* decodePrettyUnknownJson(bytes), detailed);
          assert.notInclude(bytes, FABRICATED_ATTEMPT_ID);
          assert.include(bytes, detailedAttempts[0]!.id);
        }),
    );

    it.effect(
      "rejects a measured publication without a recorded manual approval and writes nothing",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
          ...
          yield* rejection(
            exportTo(outputDirectory, measured),
            "invalid_publication",
            "measured preview",
          );
          assert.isNull(yield* readPublicEvalIndex({ outputDirectory, dataOrigin: "measured" }));
          assert.isFalse(
            yield* fs.exists(originPaths(path, outputDirectory, "measured").snapshot("rev-1")),
          );
        }),
    );

    it.effect(
      "accepts measured approval only for its exact subject and rejects private review text",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
          const { outputDirectory } = yield* canonicalOutputDirectory(
            "publication-approved-subject-",
          );
          ...
          assert.deepStrictEqual(yield* decodePrettyUnknownJson(approvedBytes), approved);
          ...
          const privateRecord = ["sk", "-proj-Qw3eRt6yUi9oPa2sDf5gHj8k"].join("");
          const privateReview = {
            ...approved,
            review: { ...approved.review, record: `Reviewed ${privateRecord}.` },
          };
          const failure = yield* rejection(
            exportTo(outputDirectory, privateReview),
            "unsafe_text",
            "private manual review record",
          );
    ...
    it.effect("rejects origin confusion between publication, result and index", () =>
      Effect.gen(function* () {
        ...
        yield* rejection(
          exportTo(outputDirectory, confused),
          "invalid_publication",
          "measured result in synthetic publication",
        );
        assert.isFalse(yield* fs.exists(originPaths(path, outputDirectory).snapshot("rev-1")));

        yield* exportTo(outputDirectory, initial);
        const syntheticIndex = yield* fs.readFileString(
          originPaths(path, outputDirectory).indexPath,
        );
    ...
    it.effect(
      "rejects revision gaps, stale supersedes and reused identifiers without changing the index",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
          ...
          for (const row of rows) {
            const candidatePath = origin.snapshot(
              row.publication.revisionId,
              row.publication.publicationId,
            );
          ...
          assert.isFalse(
            yield* fs.exists(path.join(origin.originDirectory, "pub-routing-unknown")),
          );
        }),
    );

    it.effect(
      "refuses to promote when a recorded snapshot or its index entry no longer agree",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
          ...
          const tamperings: ReadonlyArray<{
            ...
            tamper: (
              snapshot: string,
              indexPath: string,
            ) => Effect.Effect<void, PublicationTamperError>;
          }> = [
            {
              label: "snapshot bytes changed",
              tamper: (snapshot) =>
                prettyJsonFile({ ...initial, publishedAt: "2026-08-26T00:00:01.000Z" }).pipe(
                  Effect.flatMap((content) => overwriteFile(fs, snapshot, content)),
                  Effect.mapError(() => new PublicationTamperError()),
                ),
            },
            {
              label: "index publishedAt differs from the snapshot",
              tamper: (_, indexPath) =>
                readMutableIndex(fs, indexPath).pipe(
                  Effect.flatMap((index) => {
                    index.publications[0]!.revisions[0]!.publishedAt = "2026-08-26T12:00:00.000Z";
                    return prettyJsonFile(index).pipe(
                      Effect.flatMap((content) => fs.writeFileString(indexPath, content)),
                    );
                  }),
                  Effect.mapError(() => new PublicationTamperError()),
                ),
            ...
            },
            {
              ...
              tamper: (snapshot, indexPath) =>
                Effect.gen(function* () {
                  const replaced = yield* prettyJsonFile({
                    ...initial,
                    publicationId: "pub-foreign",
                  });
                  yield* overwriteFile(fs, snapshot, replaced);
                  const index = yield* readMutableIndex(fs, indexPath);
                  index.publications[0]!.revisions[0]!.sha256 = sha256Hex(replaced);
                  yield* prettyJsonFile(index).pipe(
                    Effect.flatMap((content) => fs.writeFileString(indexPath, content)),
                  );
                }).pipe(Effect.mapError(() => new PublicationTamperError())),
            },
          ];
          ...
          for (const row of tamperings) {
            ...
            yield* rejection(
              exportTo(outputDirectory, correction),
            ...
            yield* rejection(
              exportTo(outputDirectory, unrelated),
              "prior_snapshot_mismatch",
              `${row.label}: unrelated publication`,
            );
            assert.isFalse(
              yield* fs.exists(origin.snapshot("rev-unrelated", "pub-unrelated")),
              row.label,
            );
            assert.isFalse(yield* fs.exists(origin.snapshot("rev-2")), row.label);
            assert.strictEqual(
              yield* fs.readFileString(origin.indexPath),
              tamperedIndex,
              row.label,
            );
            assert.strictEqual(
              yield* fs.readFileString(origin.snapshot("rev-1")),
              tamperedSnapshot,
              row.label,
            );
    ...
    it.effect(
      "rejects symbolic links at the output directory, its ancestors and inside the origin tree",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
          ...
          yield* rejection(
            exportTo(path.join(linkedParent, "output"), initial),
            "unsafe_path",
            "symlinked ancestor",
          );
          ...
          yield* rejection(
            exportTo(originOutput, initial),
            "unsafe_path",
            "symlinked origin directory",
          );
          ...
          yield* fs.symlink(
            elsewhereSnapshots,
            path.join(publicationOrigin.originDirectory, PUBLICATION_ID),
          );
          yield* rejection(
            exportTo(publicationOutput, initial),
            "unsafe_path",
            "symlinked publication directory",
          );
          ...
          yield* rejection(
            exportTo(snapshotOutput, correction),
            "unsafe_path",
            "symlinked prior snapshot",
          );
          ...
          yield* rejection(
            readPublicEvalIndex({ outputDirectory: indexOutput, dataOrigin: "synthetic" }),
            "unsafe_path",
            "symlinked index read",
          );
          yield* rejection(
            exportTo(indexOutput, correction),
            "unsafe_path",
            "symlinked index export",
          );
    ...
    it.effect(
      "rejects identifiers that would alias or escape the snapshot layout without echoing them",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const { outputDirectory } = yield* canonicalOutputDirectory("publication-identifiers-");
          const rows: ReadonlyArray<{
            field: "publicationId" | "revisionId";
            value: string;
            reason: PublicEvalPublicationErrorReason;
          }> = [
          ...
          for (const row of rows) {
            ...
            const error = yield* rejection(
              exportTo(outputDirectory, publication),
              row.reason,
              `${row.field}=${row.value}`,
            );
    ...
    it.effect("does not overwrite an existing file at the snapshot path", () =>
      Effect.gen(function* () {
        ...
        yield* fs.makeDirectory(path.join(origin.originDirectory, PUBLICATION_ID), {
          recursive: true,
        });
        yield* fs.writeFileString(origin.snapshot("rev-1"), sentinel);

        yield* rejection(
          exportTo(outputDirectory, initial),
          "snapshot_exists",
          "occupied snapshot path",
        );
    ...
    it.effect(
      "preserves a competing snapshot created after preflight and does not promote the index",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
          const { outputDirectory } = yield* canonicalOutputDirectory(
            "publication-exclusive-create-",
          );
          const origin = originPaths(path, outputDirectory);
          ...
          yield* rejection(
            exportTo(outputDirectory, initial).pipe(
          ...
          yield* rejection(
            exportTo(outputDirectory, initial),
            "index_locked",
            "interrupted publication requires recovery",
          );
          assert.strictEqual(yield* fs.readFileString(origin.snapshot("rev-1")), competingBytes);
        }),
    );
    ...
    it.effect("refuses to export while another exporter holds the index lock", () =>
      Effect.gen(function* () {
        ...
        yield* fs.remove(origin.lockPath, { recursive: true });
        yield* exportTo(outputDirectory, initial);
    ...
    it.effect("keeps correcting, withdrawing and replacing notices beyond fifty revisions", () =>
      Effect.gen(function* () {
        ...
        for (let revision = 1; revision <= 50; revision++) {
          ...
          const bytes = yield* prettyJsonFile(publication);
          yield* fs.writeFileString(origin.snapshot(publication.revisionId), bytes, {
            mode: 0o444,
          });
          revisions.push({
        ...
        yield* prettyJsonFile(index).pipe(
          Effect.flatMap((content) => fs.writeFileString(origin.indexPath, content)),
        );
        ...
        assert.deepStrictEqual(yield* decodePrettyUnknownJson(fiftyFirstBytes), correctionFiftyOne);
        ...
        const noticeFiftyThree: PublicEvalPublication = {
          ...
          supersedes: {
            ...
            revision: 52,
            reason: "withdrawal",
            summary: "Replaced the withdrawal notice.",
          },
          content: {
            kind: "withdrawal_notice",
            reason: "privacy",
    ...
    it.effect("rejects case-folded publication and revision identifiers against live history", () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const { outputDirectory } = yield* canonicalOutputDirectory("publication-live-casefold-");
        const origin = originPaths(path, outputDirectory);
        yield* exportTo(outputDirectory, initial);
        const indexBytes = yield* fs.readFileString(origin.indexPath);
        const snapshotBytes = yield* fs.readFileString(origin.snapshot("rev-1"));

        const aliasedPublication = makePublication({
          publicationId: "Pub-Routing-Synthetic",
          revisionId: "rev-other",
          revision: 1,
          publishedAt: PUBLISHED_2,
        });
        yield* rejection(
          exportTo(outputDirectory, aliasedPublication),
          "revision_mismatch",
          "publicationId case alias",
        );
        assert.isFalse(yield* fs.exists(origin.snapshot("rev-other", "Pub-Routing-Synthetic")));

        const aliasedRevision = makePublication({
          revisionId: "REV-1",
          revision: 2,
          publishedAt: PUBLISHED_2,
          supersedes: {
        ...
        yield* rejection(
          exportTo(outputDirectory, aliasedRevision),
          "revision_mismatch",
          "live revisionId case alias",
        );
        assert.isFalse(yield* fs.exists(origin.snapshot("REV-1")));
        assert.strictEqual(yield* fs.readFileString(origin.indexPath), indexBytes);
        assert.strictEqual(yield* fs.readFileString(origin.snapshot("rev-1")), snapshotBytes);
      }),
    );
    ...
    it.effect("never reuses a removed revision identifier under a different letter case", () =>
      Effect.gen(function* () {
        ...
        yield* rejection(
          exportTo(outputDirectory, recycled),
          "revision_mismatch",
          "removed identifier reused in upper case",
        );
        assert.isFalse(yield* fs.exists(origin.snapshot("REV-1")));
        const aliasedPublication = makePublication({
          publicationId: "Pub-Routing-Synthetic",
        ...
        yield* rejection(
          exportTo(outputDirectory, aliasedPublication),
          "revision_mismatch",
          "removed publicationId reused in mixed case",
        );
        assert.isFalse(yield* fs.exists(origin.snapshot("rev-fresh", "Pub-Routing-Synthetic")));
        assert.deepStrictEqual(yield* fs.readDirectory(publicationDirectory), ["rev-2.json"]);
    ...
    it.effect(
      "a privacy withdrawal removes every historic result snapshot and leaves only the notice reachable",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
          ...
          assert.deepStrictEqual(yield* decodePrettyUnknownJson(noticeBytes), withdrawal);
          ...
          const restore = makePublication({
            ...
            supersedes: {
              ...
              revision: 3,
              reason: "correction",
              summary: "Attempted restoration.",
            },
          });
          yield* rejection(
            exportTo(outputDirectory, restore),
            "revision_mismatch",
            "restore after withdrawal",
          );
          yield* rejection(
            exportTo(outputDirectory, {
              ...initial,
              revisionId: "rev-5",
              publishedAt: PUBLISHED_4,
            }),
    ...
    it.effect("allows a privacy withdrawal to remove previously indexed private metadata", () =>
      Effect.gen(function* () {
        ...
        const privateBytes = yield* prettyJsonFile(privateSnapshot);
        yield* overwriteFile(fs, origin.snapshot("rev-2"), privateBytes);
        yield* fs.chmod(origin.snapshot("rev-2"), 0o600);
        const index = yield* readMutableIndex(fs, origin.indexPath);
        ...
        yield* rejection(
          readPublicEvalIndex({
            outputDirectory,
            dataOrigin: "synthetic",
            privacyWithdrawalPublicationId: "pub-other",
          }),
          "unsafe_text",
        ...
        yield* rejection(
          readPublicEvalIndex({
            outputDirectory,
            dataOrigin: "synthetic",
            privacyWithdrawalPublicationId: PUBLICATION_ID.toUpperCase(),
          }),
          "unsafe_text",
          "a case-folded publicationId cannot bypass the private summary check",
        );
        assert.deepStrictEqual(
          yield* readPublicEvalIndex({
            outputDirectory,
            dataOrigin: "synthetic",
            privacyWithdrawalPublicationId: PUBLICATION_ID,
          }),
          index,
        );
        ...
        const blocked = yield* rejection(
          exportTo(outputDirectory, thirdResult),
          "unsafe_text",
        ...
        if (cleanIndex === null)
          return assert.fail("index missing after private metadata withdrawal");
    ...
    it.effect(
      "replaces a newly unsafe withdrawal notice without retaining its bytes or bypassing its hash",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
          const { outputDirectory } = yield* canonicalOutputDirectory(
            "publication-private-notice-",
          );
          const origin = originPaths(path, outputDirectory);
          yield* exportTo(outputDirectory, initial);
          const firstNotice: PublicEvalPublication = {
          ...
          yield* exportTo(outputDirectory, firstNotice);
          assert.isFalse(yield* fs.exists(origin.snapshot("rev-1")));
          const privateMarker = ["sk", "-proj-Zx3cVb6nMq9wEr2tYu5iOp8a"].join("");
          ...
          const unsafeBytes = yield* prettyJsonFile(unsafeNotice);
          ...
          yield* rejection(
            exportTo(outputDirectory, replacement),
            "prior_snapshot_mismatch",
            "privacy withdrawal still checks prior notice hash",
          );
          ...
          assert.deepStrictEqual(
            yield* fs.readDirectory(path.join(origin.originDirectory, PUBLICATION_ID)),
            ["rev-3.json"],
          );
          const replacementBytes = yield* fs.readFileString(origin.snapshot("rev-3"));
          assert.deepStrictEqual(yield* decodePrettyUnknownJson(replacementBytes), replacement);
          assert.notInclude(replacementBytes, privateMarker);
          const cleanIndex = yield* readPublicEvalIndex({
            outputDirectory,
            dataOrigin: "synthetic",
          });
    ...
    it.effect(
      "rejects secrets in a correction summary or withdrawal notice without echoing them",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
          ...
          const apiKey = ["sk", "-proj-Ab3dEf6hIj9kLm2nOp5qRs8t"].join("");
          const token = [
            "eyJhbGciOiJIUzI1NiJ9",
            "eyJzdWIiOiJmaXh0dXJlIn0",
            "c2lnbmF0dXJlZml4dHVyZQ",
          ].join(".");
          const leakingCorrection: PublicEvalPublication = {
            ...
            supersedes: {
              ...
              revision: 1,
              reason: "correction",
              summary: `Rotated ${apiKey} after review.`,
            },
          };
          const leakingWithdrawal: PublicEvalPublication = {
            ...
            content: {
              ...
              reason: "privacy",
              withdrawnAt: WITHDRAWN_AT,
              notice: `Session ${token} was exposed.`,
            },
          };

          const correctionError = yield* rejection(
            exportTo(outputDirectory, leakingCorrection),
            "unsafe_text",
            "secret in summary",
          );
          assert.notInclude(errorText(correctionError), apiKey);
          const withdrawalError = yield* rejection(
            exportTo(outputDirectory, leakingWithdrawal),
            "unsafe_text",
            "secret in notice",
          );
