...
import type { PluginEvalCaseScore, PluginEvalObservation, PluginEvalObservationSet } from "../src/contracts";
import {
...
describe("public eval attempt summaries", () => {
  it.effect("excludes private observations and grader details from summaries and safe failures", () =>
    Effect.gen(function* () {
      const markers = [
      ...
      const summaries = yield* makePublicEvalAttemptSummaries(observationSet([observed]), [score]);
      ...
      const rejected = yield* Effect.result(makePublicEvalAttemptSummaries(
        observationSet([{ ...observed, token_usage: { input_tokens: -1, output_tokens: 4, total_tokens: 9 } }]),
        [score],
      ));
      ...
      if (rejected._tag === "Failure") {
        ...
        for (const marker of markers) {
          assert.notInclude(JSON.stringify(summaries), marker);
          assert.notInclude(JSON.stringify(rejected.failure), marker);
  ...
  it.effect("pairs reordered explicit identities without merging repeated case observations", () =>
    Effect.gen(function* () {
      const summaries = yield* makePublicEvalAttemptSummaries(observationSet(), [
        repeatedGrades[1], repeatedGrades[0],
      ]);
      assert.deepStrictEqual(
        [...summaries].sort((left, right) => left.repetition - right.repetition).map((attempt) => ({
          id: attempt.id,
        ...
        [
          {
            id: "attempt-c46e9a2b9ed9765cffb4896c79df276f88dfbcef247cba38e2d44c94d6843a77",
            repetition: 1, verdict: "pass", durationMs: 5, routing: "pass",
            failureCategories: [], tokenUsage: null,
          },
          {
            id: "attempt-e6c8306deade4846f581756a8af33a16938583fe2ac1bf574c2d1198477cc19b",
            repetition: 2, verdict: "fail", durationMs: 7, routing: "fail",
            failureCategories: ["routing_mismatch"], tokenUsage: null,
          },
        ],
      );
    }),
  ...
  it.effect("uses graded dimensions rather than status or error wording to classify failures", () =>
    Effect.gen(function* () {
      const summaries = yield* makePublicEvalAttemptSummaries(observationSet([
        ...
        observation(2, { status: "blocked", error: "skill activation mismatch and tool failure" }),
      ]), [
        graded(1),
        graded(2, { overall_pass: false, arguments: { score: 0, details: ["safety violation"] } }),
      ]);
      assert.deepStrictEqual(summaries.map((attempt) => ({
        verdict: attempt.verdict,
        ...
        {
          verdict: "pass",
          checks: { routing: "pass", arguments: "pass", safety: "not_applicable", completion: "pass", skillActivation: "not_applicable" },
          categories: [], validity: "valid", replacementOf: null,
        },
        {
          verdict: "fail",
          checks: { routing: "pass", arguments: "fail", safety: "not_applicable", completion: "pass", skillActivation: "not_applicable" },
          categories: ["argument_mismatch"], validity: "valid", replacementOf: null,
        },
      ]);
    }),
  );
  ...
  it.effect("rejects incomplete, foreign, out-of-range and duplicate attempt pairings", () =>
    Effect.gen(function* () {
      const invalid = [
        { name: "ungraded observation", observations: [observation(1), observation(2)], grades: [graded(1)] },
        ...
        { name: "grade from another run", observations: [observation(1)], grades: [{ ...graded(1), runId: "foreign-run" }] },
        {
          name: "grade from another case",
          observations: [observation(1)],
          grades: [{ ...graded(1, { case_id: "foreign-case" }), caseId: "foreign-case" }],
        },
        { name: "score disagrees with its identity", observations: [observation(1)], grades: [graded(1, { case_id: "foreign-case" })] },
        { name: "grade for another repetition", observations: [observation(1)], grades: [{ ...graded(1), repetition: 2 }] },
        { name: "repetition exceeds manifest", observations: [observation(3)], grades: [graded(3)] },
        { name: "duplicate observations", observations: [observation(1), observation(1)], grades: [graded(1), graded(2)] },
        { name: "duplicate grades", observations: [observation(1), observation(2)], grades: [graded(1), graded(1)] },
      ];
      for (const row of invalid) {
        const result = yield* Effect.result(makePublicEvalAttemptSummaries(observationSet(row.observations), row.grades));
        assert.strictEqual(result._tag, "Failure", row.name);
        if (result._tag === "Failure") assert.instanceOf(result.failure, PublicEvalAttemptCaptureError, row.name);
      }
    }),
  );
  ...
  it.effect("rejects invalid measurements before they can enter a public capture", () =>
    Effect.gen(function* () {
      ...
      for (const measurement of invalid) {
        ...
        const result = yield* Effect.result(makePublicEvalAttemptSummaries(
          observationSet([observed]), [graded(1, { latency_ms: observed.duration_ms })],
        ));
        assert.strictEqual(result._tag, "Failure");
        if (result._tag === "Failure") assert.instanceOf(result.failure, PublicEvalAttemptCaptureError);
      }
    }),
  );
    ...
    Effect.gen(function* () {
      ...
      for (const unsafe of unsafeIdentifiers) {
        for (const field of ["run", "case"] as const) {
          const set = observationSet([observation(1, field === "run" ? { run_id: unsafe } : { case_id: unsafe })]);
          const result = yield* Effect.result(makePublicEvalAttemptSummaries(
            field === "run" ? { ...set, manifest: { ...set.manifest, run_id: unsafe } } : set,
            [field === "run" ? { ...graded(1), runId: unsafe } : { ...graded(1, { case_id: unsafe }), caseId: unsafe }],
          ));
          ...
          if (result._tag === "Failure") {
            ...
            assert.notInclude(JSON.stringify(result.failure), JSON.stringify(unsafe).slice(1, -1));
...
describe("public eval attempt writer", () => {
  it.layer(TestPlatformLayer)((it) => {
    it.effect("saves a private companion bound to the exact sanitized report bytes and refuses overwrite", () =>
      Effect.gen(function* () {
        ...
        const capture = yield* makePublicEvalAttemptCapture({ runId: report.runId, reportContent, attempts });
        ...
        const overwrite = yield* Effect.result(writePublicEvalAttemptCapture({ outputPath, reportPath, capture }));
        ...
        if (overwrite._tag === "Failure") {
          ...
          if (overwrite.failure instanceof PublicEvalAttemptWriteError) assert.strictEqual(overwrite.failure.reason, "output-exists");
        }
    ...
    it.effect("does not clobber a companion created after preflight", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(result._tag, "Failure");
        if (result._tag === "Failure") assert.instanceOf(result.failure, PublicEvalAttemptWriteError);
    ...
    it.effect("refuses report-path aliases without changing the report", () =>
      Effect.gen(function* () {
        ...
        for (const outputPath of [reportPath, path.join(alias, "report.json")]) {
          const result = yield* Effect.result(writePublicEvalAttemptCapture({ outputPath, reportPath, capture }));
          assert.strictEqual(result._tag, "Failure");
          if (result._tag === "Failure") {
            assert.instanceOf(result.failure, PublicEvalAttemptWriteError);
            if (result.failure instanceof PublicEvalAttemptWriteError) assert.strictEqual(result.failure.reason, "output-conflict");
          }
          assert.strictEqual(yield* fs.readFileString(reportPath), reportContent);
        }
      }),
