...
describe("public eval result adapter", () => {
  it.effect("retains repeated outcomes and source token totals with a pinned declaration", () =>
    Effect.gen(function* () {
      ...
      const result = yield* makePublicEvalResult(
        options({ attemptCaptureJson, configurationJson: declaration }),
      );
      ...
      const malformed = yield* failureOf(
        options({
          expectedProvenance: {
            ...provenance,
            credential: token,
          } as PublicEvalAdapterOptions["expectedProvenance"],
        }),
      );
      assert.strictEqual(malformed.reason, "report_rejected");
      assert.notInclude(encodeJson(malformed), token);
  ...
  it.effect(
    "rejects attempt slots that are misidentified, duplicated, missing or out of range",
    () =>
      Effect.gen(function* () {
        ...
        const rows = [
          {
            ...
            attempts: [
              ...
              second,
              third,
              { ...fourth, id: attemptId("synthetic-other-run", "argument-shape", 2) },
            ],
  ...
  it.effect(
    "rejects a companion whose verdicts, checks, latency ranks or token sums disagree with the aggregate",
    () =>
      Effect.gen(function* () {
        const [first, second, third, fourth] = attempts;
        const rows = [
          ...
          },
          {
            ...
            attempts: [
              { ...first, tokenUsage: { inputTokens: 3, outputTokens: 4, totalTokens: 10 } },
              second,
              third,
              fourth,
            ],
    ...
    Effect.gen(function* () {
      ...
      const rows = [
        ...
        },
        {
          ...
          input: options({
            configurationJson: configurationJson({
              settings: { ...runSettings, accountClass: apiKey },
            }),
          }),
        ...
        },
        {
          ...
          input: options({
            configurationJson: configurationJson({
              settings: { ...runSettings, accountClass: address },
            }),
          }),
          reason: "invalid_configuration",
        },
        {
          ...
          input: options({
            configurationJson: configurationJson({
              settings: { ...runSettings, accountClass: privateHost },
            }),
          }),
          reason: "invalid_configuration",
        },
      ...
      for (const row of rows) {
        ...
        assert.strictEqual(failure.reason, row.reason, row.name);
        assert.notInclude(encodeJson(failure), row.marker, row.name);
        assert.notInclude(String(failure), row.marker, row.name);
      }
    }),
  );

  it.effect("rejects nonzero source token totals without token observations", () =>
    Effect.gen(function* () {
      const rows = [
        {
          name: "input total",
          tokenUsage: { observations: 0, inputTokens: 1, outputTokens: 0, totalTokens: 0 },
        },
        {
          name: "output total",
          tokenUsage: { observations: 0, inputTokens: 0, outputTokens: 1, totalTokens: 0 },
        },
        {
          name: "combined total",
          tokenUsage: { observations: 0, inputTokens: 0, outputTokens: 0, totalTokens: 1 },
        },
      ] as const;
      for (const row of rows) {
        const inconsistent = {
          ...report,
          aggregate: { ...report.aggregate, tokenUsage: row.tokenUsage },
        };
        const failure = yield* failureOf(
          options({ reportJson: serialize(inconsistent) }),
          row.name,
        );
        assert.strictEqual(failure.reason, "invalid_report", row.name);
      }
    }),
  );

  it.effect(
    "keeps unmeasured token and financial metrics unavailable and derives unique cases from repetitions",
    () =>
      Effect.gen(function* () {
        const untracked = {
          ...report,
        ...
        const measuredZero = {
          ...
          aggregate: {
            ...report.aggregate,
            tokenUsage: { observations: 2, inputTokens: 0, outputTokens: 0, totalTokens: 0 },
          },
        };
        const measuredZeroJson = serialize(measuredZero);
        const zeroTokenAttempts = attempts.map((entry) =>
          entry.tokenUsage === null
            ? entry
            : { ...entry, tokenUsage: { inputTokens: 0, outputTokens: 0, totalTokens: 0 } },
        );
        const zeroResult = yield* makePublicEvalResult(
          options({
            reportJson: measuredZeroJson,
            attemptCaptureJson: captureJson(measuredZeroJson, zeroTokenAttempts),
          }),
        );
        assert.deepStrictEqual(zeroResult.metrics.tokenUsage, {
          availability: "available",
          unit: "tokens",
          inputTokens: 0,
          outputTokens: 0,
        ...
        assert.deepStrictEqual(result.metrics.usdCost, {
          availability: "not_evaluated",
          reason: "no_declared_method",
        });
        assert.deepStrictEqual(result.metrics.answerAccuracy, {
          availability: "not_evaluated",
          reason: "no_declared_method",
        });
        assert.deepStrictEqual(result.metrics.uncertainty, {
          availability: "not_evaluated",
          reason: "no_declared_method",
        });
        assert.deepStrictEqual(result.counts, {
  ...
  it.effect("validates a withheld companion fully and withholds only the retained detail", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(result.source.kind, "sanitized_aggregate");
      assert.strictEqual(result.source.attemptCaptureSha256, null);
      assert.deepStrictEqual(result.counts.cases, {
        total: 2,
        passedEveryAttempt: null,
        failedAnyAttempt: null,
      });
      assert.strictEqual(result.metrics.passRate.availability, "available");

      const [first, second, third, fourth] = attempts;
      const stale = yield* failureOf(
        options({
          attemptCaptureJson: captureJson(reportJson, [
            first,
            second,
  ...
  it.effect("withholds the headline while an authoritative plan is incomplete", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(result.metrics.passRate, {
      ...
      const contradictory = yield* failureOf(
        options({
          declaredCoverage: {
            plannedCases: 3,
            plannedAttempts: 5,
    ...
    () =>
      Effect.gen(function* () {
        ...
        for (const mismatch of mismatches) {
          const overstated = yield* failureOf(
            options({ configurationJson: configurationJson(mismatch) }),
          );
          assert.strictEqual(overstated.reason, "configuration_mismatch", encodeJson(mismatch));
        }
        ...
        for (const malformed of malformedRows) {
          const unsupported = yield* failureOf(
            options({ configurationJson: configurationJson(malformed) }),
          );
          assert.strictEqual(unsupported.reason, "invalid_configuration", encodeJson(malformed));
        }
