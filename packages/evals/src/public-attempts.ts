...
const decodeSummaries = Schema.decodeUnknownEffect(
  Schema.Array(PublicEvalAttemptSummarySchema), PUBLIC_EVAL_DECODE_OPTIONS,
);
const decodeReport = Schema.decodeUnknownEffect(
  Schema.fromJsonString(SanitizedEvalRunReportSchema), PUBLIC_EVAL_DECODE_OPTIONS,
);
const encodeCapture = Schema.encodeEffect(
  Schema.fromJsonString(PublicEvalAttemptCaptureSchema, { space: 2 }),
);
const captureError = () => new PublicEvalAttemptCaptureError({
...
const checkVerdict = (dimension: PluginEvalDimensionScore | undefined) =>
  dimension === undefined ? "not_applicable" as const : dimension.score === 1 ? "pass" as const : "fail" as const;
...
export const assertPublicEvalAttemptPlan = (
  runId: string,
  caseIds: readonly string[],
  repetitions: number,
): Effect.Effect<void, PublicEvalAttemptCaptureError> => Effect.gen(function* () {
  if (
    !Number.isSafeInteger(repetitions) || repetitions < 1 || caseIds.length === 0 ||
    caseIds.length > PUBLIC_EVAL_MAX_ATTEMPTS / repetitions ||
    !isPublicIdentifier(runId) || !isSafePublicEvalText(runId)
  ) return yield* captureError();
  const seen = new Set<string>();
  for (const caseId of caseIds) {
    if (
      seen.has(caseId) || !isPublicIdentifier(caseId) || !isSafePublicEvalText(caseId)
    ) return yield* captureError();
    seen.add(caseId);
  }
});
...
export const makePublicEvalAttemptSummaries = (
  ...
  Effect.gen(function* () {
    ...
    if (
      observations.length === 0 || observations.length > PUBLIC_EVAL_MAX_ATTEMPTS ||
      observations.length !== gradedAttempts.length ||
      !Number.isSafeInteger(manifest.repetitions) || manifest.repetitions < 1
    ) {
      return yield* captureError();
    }
    const byIdentity = new Map<string, (typeof observations)[number]>();
    for (const observation of observations) {
      const key = publicEvalAttemptIdentityInput(observation.run_id, observation.case_id, observation.repetition);
      if (
        byIdentity.has(key) || observation.run_id !== manifest.run_id ||
        observation.target !== manifest.target || observation.model !== manifest.model ||
        !Number.isSafeInteger(observation.repetition) || observation.repetition < 1 ||
    ...
    for (const graded of gradedAttempts) {
      ...
      if (
        observation === undefined || score.case_id !== graded.caseId ||
        score.latency_ms !== observation.duration_ms || typeof score.overall_pass !== "boolean" ||
        score.routing === undefined || score.arguments === undefined || score.completion === undefined ||
        hasInvalidGrade(score.routing) || hasInvalidGrade(score.arguments) ||
        hasInvalidGrade(score.safety) || hasInvalidGrade(score.completion) ||
        hasInvalidGrade(score.skill_activation) ||
        !isSafePublicEvalText(graded.runId) || !isSafePublicEvalText(graded.caseId)
      ) return yield* captureError();
      ...
      attempts.push({
        ...
        tokenUsage: observation.token_usage === undefined ? null : {
...
export const makePublicEvalAttemptCapture = ({
  runId, reportContent, attempts,
}: PublicEvalAttemptCaptureSource): Effect.Effect<PublicEvalAttemptCapture, PublicEvalAttemptCaptureError> =>
  Effect.gen(function* () {
    ...
    if (
      report.runId !== runId || capture.attempts.length !== report.aggregate.overall.total ||
    ...
    for (const attempt of capture.attempts) {
      if (
        ...
        attempt.id !== `attempt-${sha256Hex(publicEvalAttemptIdentityInput(runId, attempt.caseId, attempt.repetition))}` ||
...
export const assertPublicEvalAttemptOutputPath = (
  outputPath: string,
  reportPath: string,
  ...
  Effect.gen(function* () {
    ...
    const canonical = (input: string) => Effect.gen(function* () {
    ...
    if (output === report || output.startsWith(`${report}${path.sep}`) || report.startsWith(`${output}${path.sep}`)) {
      return yield* new PublicEvalAttemptWriteError({ reason: "output-conflict" });
    }
...
export const writePublicEvalAttemptCapture = ({
  outputPath, reportPath, capture,
}: PublicEvalAttemptCaptureWriteOptions): Effect.Effect<
  void, PublicEvalAttemptCaptureError | PublicEvalAttemptWriteError, FileSystem.FileSystem | Path.Path
> =>
  Effect.gen(function* () {
    ...
    const path = yield* Path.Path;
    const resolvedOutput = path.resolve(outputPath);
    const reportBytes = yield* fs.readFile(path.resolve(reportPath)).pipe(Effect.mapError(writeError));
    ...
    const validated = yield* makePublicEvalAttemptCapture({
      runId: capture.runId, reportContent, attempts: capture.attempts,
    });
    ...
    yield* fs.makeDirectory(path.dirname(resolvedOutput), { recursive: true }).pipe(Effect.mapError(writeError));
    yield* fs.writeFileString(resolvedOutput, `${encoded}\n`, { flag: "wx", mode: 0o600 }).pipe(Effect.mapError(writeError));
  });
