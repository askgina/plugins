...
const error = (reason: PublicEvalPublicationErrorReason) =>
  new PublicEvalPublicationError({ reason });
const encodePublicationJson = Schema.encodeEffect(
  Schema.fromJsonString(PublicEvalPublicationSchema, { space: 2 }),
);
const encodeIndexJson = Schema.encodeEffect(
  Schema.fromJsonString(PublicEvalIndexSchema, { space: 2 }),
);
...
export const PublicEvalExportRequestSchema = Schema.Union([
  Schema.Struct({
    ...
    declaredCoverage: Schema.optionalKey(
      Schema.Struct({
  ...
  Schema.Struct({
    ...
    notice: Schema.Struct({
      ...
      notice: Schema.NonEmptyString.check(
  ...
  Schema.makeFilter((request) => {
    if ((request.dataOrigin === "measured") !== (request.review.status === "approved"))
      return "origin requires its matching review";
    if ((request.revision === 1) !== (request.supersedes === null))
      return "revision requires its predecessor";
    if (
      request.review.status === "approved" &&
      DateTime.isGreaterThan(
        DateTime.makeUnsafe(request.review.approvedAt),
        DateTime.makeUnsafe(request.publishedAt),
      )
    )
      return "approval must precede publication";
    if (
    ...
    if (
      ...
      (request.supersedes === null ||
        DateTime.isGreaterThan(
          DateTime.makeUnsafe(request.notice.withdrawnAt),
          DateTime.makeUnsafe(request.publishedAt),
        ))
    )
...
export type PublicEvalWithdrawalExportRequest = Extract<
  PublicEvalExportRequest,
  { kind: "withdrawal" }
>;

export const decodePublicEvalExportRequest = (
  input: unknown,
): Effect.Effect<PublicEvalExportRequest, PublicEvalPublicationError> =>
  Schema.decodeUnknownEffect(
    ...
    Effect.flatMap((request) =>
      hasSafePublicEvalFields(request)
...
export const publicEvalPublicationSubjectSha256 = (
  publication: Omit<PublicEvalPublication, "review">,
): string =>
  createHash("sha256")
    .update(
      canonicalJson(
...
const validatePublication = (
  input: unknown,
  deletingPrivateBytes = false,
): Effect.Effect<PublicEvalPublication, PublicEvalPublicationError> =>
  decodePublicEvalPublication(input).pipe(
    Effect.mapError(() => error("invalid_publication")),
    Effect.flatMap((publication) => {
      if (!deletingPrivateBytes && !hasSafePublicEvalFields(publication))
        return Effect.fail(error("unsafe_text"));
      if (
        publication.review.status === "approved" &&
        publication.review.subjectSha256 !== publicEvalPublicationSubjectSha256(publication)
      ) {
...
export const makePublicEvalResultPublication = Function.dual<
  (
...
>(2, (request, result) =>
  Effect.gen(function* () {
    ...
    if (decoded.kind !== "result" || decoded.resultId !== result.resultId)
      return yield* error("invalid_request");
...
export const makePublicEvalWithdrawalPublication = Function.dual<
  (
    runId: string,
  ) => (
...
const snapshotPath = (publicationId: string, revisionId: string): string =>
  `${publicationId}/${revisionId}.json`;
...
const inspectPath = (
  target: string,
): Effect.Effect<FileSystem.File.Info | null, PublicEvalPublicationError, Platform> =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const components: string[] = [];
    for (let next = target; path.dirname(next) !== next; next = path.dirname(next))
...
const ensureDirectory = (
  target: string,
): Effect.Effect<void, PublicEvalPublicationError, Platform> =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    ...
    if (existing === null)
      yield* fs
        .makeDirectory(target, { recursive: true, mode: 0o755 })
        .pipe(Effect.mapError(() => error("write_failed")));
...
): Effect.Effect<Layout, PublicEvalPublicationError, Platform> =>
  Effect.gen(function* () {
    const path = yield* Path.Path;
    if (
      ...
      (dataOrigin !== "synthetic" && dataOrigin !== "measured")
    ) {
      return yield* error("unsafe_path");
    }
...
const readIndex = (
  layout: Layout,
  dataOrigin: PublicEvalDataOrigin,
  privacyWithdrawalPublicationId?: string,
): Effect.Effect<PublicEvalIndex | null, PublicEvalPublicationError, Platform> =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    ...
    const bytes = yield* fs
      .readFile(layout.indexPath)
      .pipe(Effect.mapError(() => error("index_unreadable")));
    const text = yield* Effect.try({
      try: () => new TextDecoder("utf-8", { fatal: true, ignoreBOM: true }).decode(bytes),
      catch: () => error("index_invalid"),
    });
    const index = yield* decodeJson(text).pipe(
      Effect.flatMap(decodePublicEvalIndex),
      Effect.mapError(() => error("index_invalid")),
    );
    ...
    const retained =
      privacyWithdrawalPublicationId === undefined
        ? index
        : {
            ...index,
            publications: index.publications.map((entry) =>
              entry.publicationId === privacyWithdrawalPublicationId
                ? { ...entry, summary: null, review: null }
                : entry,
            ),
          };
    if (!hasSafePublicEvalFields(retained)) return yield* error("unsafe_text");
    return index;
  });
...
const summaryOf = (publication: PublicEvalPublication): IndexEntry["summary"] =>
  publication.content.kind === "result"
    ? {
...
const verifyIndexSnapshots = (
  ...
  Effect.gen(function* () {
    ...
    for (const entry of index.publications) {
      if (!PATH_SEGMENT.test(entry.publicationId) || /^index(?:\.|$)/iu.test(entry.publicationId))
        return yield* error("unsafe_path");
      ...
      for (const revision of entry.revisions) {
        ...
        if (
          revision.path !== expected ||
          info?.type !== "File" ||
          Option.getOrElse(info.nlink, () => 1) !== 1
        )
          return yield* error("prior_snapshot_mismatch");
        expectedFiles.add(`${revision.revisionId}.json`);
        const bytes = yield* fs
          .readFile(target)
          .pipe(Effect.mapError(() => error("prior_snapshot_mismatch")));
        ...
        if (
          prior.publicationId !== entry.publicationId ||
          prior.runId !== entry.runId ||
          prior.dataOrigin !== index.dataOrigin ||
          prior.revisionId !== revision.revisionId ||
          prior.revision !== revision.revision ||
          prior.content.kind !== revision.kind ||
          prior.publishedAt !== revision.publishedAt ||
          (predecessor !== undefined &&
            (prior.supersedes?.revisionId !== predecessor.revisionId ||
              prior.supersedes?.revision !== predecessor.revision ||
              DateTime.isLessThan(
                DateTime.makeUnsafe(prior.publishedAt),
                DateTime.makeUnsafe(predecessor.publishedAt),
              ))) ||
          (revision.state === "current" &&
            (canonicalJson(summaryOf(prior)) !== canonicalJson(entry.summary) ||
              canonicalJson(prior.review) !== canonicalJson(entry.review)))
        )
          return yield* error("prior_snapshot_mismatch");
      }
      const files = yield* fs
        .readDirectory(path.join(layout.originDirectory, entry.publicationId))
        .pipe(Effect.mapError(() => error("prior_snapshot_mismatch")));
 ...
 */
export const exportPublicEvalPublication = (
  options: PublicEvalPublicationExportOptions,
): Effect.Effect<PublicEvalPublicationExportResult, PublicEvalPublicationError, Platform> =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const publication = yield* validatePublication(options.publication);
    if (
      !PATH_SEGMENT.test(publication.publicationId) ||
      /^index(?:\.|$)/iu.test(publication.publicationId) ||
      !PATH_SEGMENT.test(publication.revisionId)
    )
      return yield* error("unsafe_path");
    ...
    const content = `${yield* encodePublicationJson(publication).pipe(
      Effect.mapError(() => error("invalid_publication")),
    )}\n`;
    ...
    return yield* Effect.acquireUseRelease(
      fs
        .makeDirectory(layout.lockPath, { mode: 0o700 })
        .pipe(
          Effect.mapError((cause) =>
            error(cause.reason._tag === "AlreadyExists" ? "index_locked" : "write_failed"),
          ),
        ),
      () =>
        Effect.gen(function* () {
          const privacyWithdrawalId =
            publication.content.kind === "withdrawal_notice" &&
            publication.content.reason === "privacy"
          ...
          const publicationIdKey = publication.publicationId.toLowerCase();
          const revisionIdKey = publication.revisionId.toLowerCase();
          if (
            entries.some(
              (item) =>
                item.publicationId.toLowerCase() === publicationIdKey &&
                item.publicationId !== publication.publicationId,
            ) ||
            entries.some((item) =>
              item.revisions.some(
                (revision) => revision.revisionId.toLowerCase() === revisionIdKey,
              ),
            )
          )
            return yield* error("revision_mismatch");
          ...
          if (entry === undefined) {
            if (publication.revision !== 1 || withdrawal)
              return yield* error("unknown_publication");
            ...
            if (
              (!withdrawal && entry.status === "withdrawn") ||
              entry.runId !== publication.runId ||
              publication.revision !== prior.revision + 1 ||
              publication.supersedes?.revisionId !== entry.currentRevisionId ||
              publication.supersedes.revision !== prior.revision ||
              DateTime.isLessThan(
                DateTime.makeUnsafe(publication.publishedAt),
                DateTime.makeUnsafe(prior.publishedAt),
              )
            )
              return yield* error("revision_mismatch");
          }
          if ((yield* inspectPath(publicationPath)) !== null)
            return yield* error("snapshot_exists");
          const revisions: IndexRevision[] = (entry?.revisions ?? []).map((revision) =>
            withdrawal
          ...
          const nextEntry: IndexEntry = {
            ...
            status: withdrawal ? "withdrawn" : "current",
            currentRevisionId: publication.revisionId,
            summary: summaryOf(publication),
            revisions,
          };
          const nextIndex = yield* decodePublicEvalIndex({
            schemaVersion: "eval-index.v1",
            dataOrigin: publication.dataOrigin,
          ...
          if (!hasSafePublicEvalFields(nextIndex)) return yield* error("unsafe_text");
          const nextContent = `${yield* encodeIndexJson(nextIndex).pipe(
            Effect.mapError(() => error("index_invalid")),
          )}\n`;
          ...
          yield* fs
            .writeFileString(publicationPath, content, { flag: "wx", mode: 0o444 })
            .pipe(
              Effect.mapError((cause) =>
                error(cause.reason._tag === "AlreadyExists" ? "snapshot_exists" : "write_failed"),
              ),
            );
          yield* fs
            .writeFileString(temporaryIndex, nextContent, { flag: "wx", mode: 0o644 })
            .pipe(Effect.mapError(() => error("write_failed")));
          ...
          yield* fs
            .rename(temporaryIndex, layout.indexPath)
            .pipe(Effect.mapError(() => error("write_failed")));
          complete = true;
      ...
      () =>
        !mutationStarted || complete
          ? fs
              .remove(layout.lockPath, { recursive: true })
              .pipe(Effect.mapError(() => error("write_failed")))
          : Effect.void,
    );
  });
